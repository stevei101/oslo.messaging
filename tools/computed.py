...
def notifier(_id, transport, messages, wait_after_msg, timeout):
    ...
    for i in range(0, (messages)):
        ...
        # n1.info(ctxt, 'image.create', _image_payload())
        # n1.info(ctxt, 'image.delete', _image_payload())
        if wait_after_msg > 0:
            time.sleep(wait_after_msg)
...
def _image_payload():
    return {
            'owner_id': six.text_type(uuid.uuid4()),
            'image_id': six.text_type(uuid.uuid4()),
            'receiver_tenant_id': six.text_type(uuid.uuid4()),
            'receiver_user_id': six.text_type(uuid.uuid4()),
            'bytes_sent': 1024,
